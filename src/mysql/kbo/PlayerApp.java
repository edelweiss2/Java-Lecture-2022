...
public class PlayerApp {
	...
	public static void registerPlayer() {
		...
		Player np = new Player(backNumber, name, position, LocalDate.parse(bday), height, 0);
		dao.insertPlayer(np);
	...
	public static void updatePlayer() {
		System.out.print("백넘버> ");
		int backNumber = Integer.parseInt(scan.nextLine());
		Player p = dao.getPlayer(backNumber);
		...
		System.out.print("생년월일(" + p.getBday() + ")> ");
		String bday = scan.nextLine();
		bday = (bday.length() == 0) ? p.getBday().toString() : bday;
		
		System.out.print("신장(" + p.getHeight() + ")> ");
		String h = scan.nextLine();
		int height = (h.length() == 0) ? p.getHeight() : Integer.parseInt(h);
		
		p = new Player(backNumber, name, position, LocalDate.parse(bday), height, 0);
		dao.updatePlayer(p);
